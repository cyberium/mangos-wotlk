...
PatchHandler::PatchHandler(protocol::Socket& socket, ACE_HANDLE patch) : m_socket(socket), m_timer(m_socket.get_io_service(), boost::posix_time::seconds(1)), patch_fd_(patch), m_sendBuffer(sizeof(Chunk))
{
...
void PatchHandler::on_write_complete(const boost::system::error_code& error, size_t bytes_transferred)
{
    if (error)
        return;
